...
rule all:
    input:
        ...
        ancient(S3.remote(expand(f'{BUCKET}/data/vcfs/joint/imputed/MNEc2M:WGS/{{fltr}}/{{maf}}/VQSR{{vqsr}}/{{sample}}/{{contig}}/sample_imp.vcf.gz',caller=caller,maf=maf,contig=contigs,fltr=fltr,vqsr=vqsr,sample=samples),keep_local=True)),
        #S3.remote(expand('{target}',target=config['targets']))

# samples = defaultdict(lambda: "Unknown", { k:v for k,v in map(lambda x: x.strip().split('\t'), open("Samples.csv").readlines()[1:])})
# conc = []                           
# for f in glob.glob("*concord.txt"): 
#     # ADR420.NC_009168_3.concord.txt
#     ind,chrom,*junk = f.split('.')  
#     x = pd.read_table(f)            
#     x['chrom'] = chrom              
#     x['breed'] = samples[ind]       
#     conc.append(x)                  
...
#    output: 
#        vcf = S3.remote(f'{BUCKET}/data/vcfs/joint/bcftools/biallelic/{{maf}}/{{contig}}/ALL.vcf.gz',keep_local=True)
#    resources:
#        disk_gb = 100,
#        mem_gb = 10
#    params:
...
#    output: 
#        vcf = S3.remote(f'{BUCKET}/data/vcfs/joint/gatk/biallelic/{{maf}}/{{contig}}/ALL.vcf.gz',keep_local=True)
#    resources:
#        disk_gb = 500,
#        mem_gb = 50
#    params:
...
#    input:
#        fdict  = S3.remote(f'{BUCKET}/data/fna/GCF_002863925.1_EquCab3.0_genomic.fna.dict',keep_local=True)
#    output:
#        fdict  = S3.remote(f'{BUCKET}/data/fna/GCF_002863925.1_EquCab3.0_genomic.dict',keep_local=True)
#    shell:
#        'cp {input[0]} {output[0]}'
