...
rule all:
    input:
        ...
        S3.remote(expand(f'{BUCKET}/data/vcfs/joint/imputed/MNEc2M:WGS/{{fltr}}/{{maf}}/VQSR{{vqsr}}/{{sample}}/{{contig}}/sans_sample.vcf.gz',caller=caller,maf=maf,contig=contigs,fltr=fltr,vqsr=vqsr,sample=samples),keep_local=True),
        S3.remote(expand(f'{BUCKET}/data/vcfs/joint/imputed/MNEc2M:WGS/{{fltr}}/{{maf}}/VQSR{{vqsr}}/{{sample}}/{{contig}}/sample_ref.vcf.gz',caller=caller,maf=maf,contig=contigs,fltr=fltr,vqsr=vqsr,sample=samples),keep_local=True),
        S3.remote(expand(f'{BUCKET}/data/vcfs/joint/imputed/MNEc2M:WGS/{{fltr}}/{{maf}}/VQSR{{vqsr}}/{{sample}}/{{contig}}/sample_flt.vcf.gz',caller=caller,maf=maf,contig=contigs,fltr=fltr,vqsr=vqsr,sample=samples),keep_local=True),
...
rule filter_individual_and_impute:
    input:
        vcf = S3.remote(f'{BUCKET}/data/vcfs/joint/merged/{{fltr}}/{{maf}}/{{contig}}/VQSR{{vqsr}}/PHASED.vcf.gz'),
        idx = S3.remote(f'{BUCKET}/data/vcfs/joint/merged/{{fltr}}/{{maf}}/{{contig}}/VQSR{{vqsr}}/PHASED.vcf.gz.csi'),
        lst = S3.remote(f'{BUCKET}/data/lsts/MNEc2M.lst') 
    output:
        sans_sample = S3.remote(f'{BUCKET}/data/vcfs/joint/imputed/MNEc2M:WGS/{{fltr}}/{{maf}}/VQSR{{vqsr}}/{{sample}}/{{contig}}/sans_sample.vcf.gz'),
    ...
    params:
        imputed     = f'{BUCKET}/data/vcfs/joint/imputed/MNEc2M:WGS/{{fltr}}/{{maf}}/VQSR{{vqsr}}/{{sample}}/{{contig}}/sample_imp',
    shell:
        f'''
            # split out sample from reference population
            bcftools view {{input.vcf}} --samples ^{{wildcards.sample}} -Oz -o {{output.sans_sample}}
            bcftools view {{input.vcf}} --samples  {{wildcards.sample}} -Oz -o {{output.sample_ref}}
            bcftools index {{output.sample_ref}}

            # Down sample the single sample and remove the phase
            bcftools view {{output.sample_ref}} -R {{input.lst}} | sed '/^##/! s/|/\//g' | bcftools view -Oz -o {{output.sample_flt}}

            # impute 
            java -jar $BEAGLE_JAR gt={{output.sample_flt}} ref={{output.sample_ref}} impute=true out={{params.imputed}}
        '''
