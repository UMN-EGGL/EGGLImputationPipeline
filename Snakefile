...
rule all:
    input:
        ...
        S3.remote(
            expand(
                '{bucket}/vcfs/{contig}/step02/{fltr}_{maf}_{caller}_{vqsr}.WGS.vcf.gz',
                bucket=config['bucket'],  caller=config['caller'], 
                maf=config['maf'],        fltr=config['fltr'],       
...
rule filter_joint_bcftools_biallelic:
    ...
    output: 
        vcf = S3.remote('{bucket}/vcfs/{contig}/step01/biallelic_{maf}_bcftools.WGS.vcf.gz')
    resources:
        disk_gb = 100,
        mem_gb = 10
    params:
    ...
    run:
        if wildcards.maf == 'MAF01':
            ...
            min_af = '0.005'
        tmpdir = f"{config['tmp_dir']}/{wildcards.contig}/biallelic_{wildcards.maf}_bcftools/"
        shell(f'''
            mkdir -p {tmpdir}
...
rule filter_joint_gatk_biallelic:
    ...
    output: 
        vcf = S3.remote('{bucket}/vcfs/{contig}/step01/biallelic_{maf}_gatk.WGS.vcf.gz')
    resources:
        disk_gb = 500,
        mem_gb = 50
    params:
    ...
    run:
        if wildcards.maf == 'MAF01':
            ...
            min_af = '0.005'
        tmpdir = f"{config['tmp_dir']}/{wildcards.contig}/biallelic_{wildcards.maf}_gatk/"
        shell(f'''
            mkdir -p {tmpdir}
            bcftools view -m2 -M2 -v snps,indels --min-af {min_af} {{input.gvcf}} -Ou | \
            bcftools norm -m+any -Ou | \
            bcftools sort -T {tmpdir} -m {{params.max_mem}} -O z -o {{output.vcf}} 
            rm -fr {config['tmp_dir']}/{wildcards.contig}
        ''')
...
rule filter_VCF_to_LST_SNPs:
    input:
        ...
        lst = S3.remote('{bucket}/lsts/{lst}.lst')
    output: 
        snps = S3.remote('{bucket}/vcfs/{contig}/step02/{fltr}_{maf}_{caller}.{lst}.vcf.gz')
    shell:
        '''
...
rule apply_VQSR_filters_vcf:
    input:
        ...
        fdict  = S3.remote('{bucket}/fna/GCF_002863925.1_EquCab3.0_genomic.dict')
    output:
        vcf = S3.remote('{bucket}/vcfs/{contig}/step02/{fltr}_{maf}_{caller}_{vqsr}.WGS.vcf.gz'),
    run:
        import tempfile
